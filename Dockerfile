...
RUN if grep -q '^ID.*rhel' /etc/os-release; then yum -y install gpg python3 python3-pip python3-libselinux && yum clean all; \
    elif grep -q '^ID.*debian' /etc/os-release; then apt-get -q update && DEBIAN_FRONTEND="noninteractive" apt-get -q install -y -o Dpkg::Options::="--force-confnew" --no-install-recommends gpg gpg-agent python3 python3-pip python3-debian && rm -rf /var/lib/apt/lists/*; \
